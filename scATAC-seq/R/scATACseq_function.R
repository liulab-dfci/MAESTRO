...
FripPlot_micro <- function(matrix, prefix, reads_cutoff = 1000, frip_cutoff = 0.05)
{
  ...
  dev.off()
  write.table(rownames(matrix[which(matrix[,5]>=reads_cutoff&(matrix[,6]/matrix[,5])>=frip_cutoff),]), paste0("Result/QC/",prefix,"_barcodes.txt"), sep = "\n", quote=F, row.names=F, col.names=F)
}
...
FripPlot_10x <- function(matrix, prefix, reads_cutoff = 1000, frip_cutoff = 0.05)
{
  ...
  legend("topright",c("cells","non-cells"),col=c("red","blue"),pch=20,box.lty=0)
  dev.off()
  write.table(as.character(matrix[which(matrix[,8]>=reads_cutoff&(matrix[,14]/matrix[,8])>=frip_cutoff),1]), paste0("Result/QC/",prefix,"_barcodes.txt"), sep = "\n", quote=F, row.names=F, col.names=F)
}
