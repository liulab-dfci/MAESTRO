...
def FP(args):
    ...
    bg_peak_len = []
    bg_peak_sets = []
    for i in range(0,100):                                   # generate 100 bg sets peaks
        bg_peak = generate_bg_peaks(all_peak, sc_peak_set)
        bg_peak_sets.append(bg_peak)
        bg_peak_len.append(len(bg_peak))
    pvalue = []
    bg_len_mean = numpy.mean(bg_peak_len)
    for tf in tf_list:
        sc_tf = len(sc_peak_set & tf_peak[tf])
        bg_tf = []
        for bg_peak in bg_peak_sets:
            bg_tf.append(len(bg_peak & tf_peak[tf]))
        bg_tf_mean = numpy.mean(bg_tf)
        pvalue.append(str(-1*math.log(stats.fisher_exact([[sc_len - sc_tf, sc_tf], [bg_len_mean - bg_tf_mean, bg_tf_mean]])[1]+1E-300,10)))
    
    return(pvalue)
    
def ZS(args):
    """Multiple processing function to calculate zscore distribution."""
    ...
    for tf in tf_list:
        ...
        for bg_peak in bg_peak_sets:
            bg_tf.append(len(bg_peak & tf_peak[tf]))
        zscore.append(str((sc_tf-numpy.mean(bg_tf))/(numpy.std(bg_tf)+0.0001)))
