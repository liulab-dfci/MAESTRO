...
#' @param type Type of the dataset. If \code{type} is "RNA" or "Incorparated", the expression level of TFs will be visualized. 
#' If \code{type} is "ATAC", the regulatory potential will be used for visualization. Default is "RNA".
#' @param SeuratObj A Seurat object. If \code{type} is "RNA"or "Incorparated", \code{SeuratObj} should be the Seurat object generated by \code{\link{RNARunSeurat}} function.
...
VisualizeTFenrichment <- function(TFs = NULL, cluster.1, cluster.2 = NULL, type = "RNA", SeuratObj, visual.topnumber = 10, visual.totalnumber = 100, LISA.table = NULL, GIGGLE.table = NULL, width = 5, height = 4.5, name = "TFEnrichmentPlot"){
  ...
  if(type == "Integrated")
  {
    ...
    TFmean <- apply(log2(SeuratObj$RNA@counts[sg,SeuratObj@meta.data[,'RNA_snn_res.0.6']==as.character(cluster.1)]+1),1,mean)+0.0000000001 # expression uing scRNA-seq
    TFmean <- scale((TFmean/abs(TFmean))*(abs(TFmean)^0.3),scale=FALSE)
