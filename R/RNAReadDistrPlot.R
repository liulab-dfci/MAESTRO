...
RNAReadDistrPlot <- function(bamstat.filepath, readdistr.filepath, name){
  ...
  bam_stat_list = readLines(bamstat.filepath)[c(6,9,14)]
  bam_stat_list = sapply(bam_stat_list, function(x){
    return(sub(":\\s+",":",x))
  })
  names(bam_stat_list) = NULL
  bam_stat_list = as.integer(unlist(strsplit(bam_stat_list, split = ":"))[seq(2,2*length(bam_stat_list),2)])
  ...
  Group = c("Total reads", "Duplicate reads", "Unique mapped reads","Exon reads", "Intron reads")
