...
RNAAnnotateTranscriptionFactor <- function(RNA, genes, project, method = "RABIT" ,rabit.path, organism = "GRCh38", top.tf = 10)
{
  require(Seurat)
  require(MAGeCKFlute)
  require(Matrix)
  ...
  if(method == "RABIT"){
    ...
    out_fdr_max_log = tryCatch(expr = RunRABIT(genes = genes, project = project, rabit.path = rabit.path, organism = organism),
...
RunRABIT <- function(genes, project, rabit.path, organism = "GRCh38"){
