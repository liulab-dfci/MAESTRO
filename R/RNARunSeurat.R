...
RNARunSeurat <- function(inputMat, type = "matrix", project = "MAESTRO.scRNA.Seurat", orig.ident = NULL, min.c = 10, min.g = 200, mito = FALSE,
                         mito.cutoff = 20, variable.genes = 2000, organism = "GRCh38", dims.use = 1:15, cluster.res = 0.6, only.pos = FALSE, genes.test.use = "presto",
                         ...
                         findclusters.args = list(), outdir = ".", ...)
{
  ...
  if(mito){
    ...
    if(organism=="GRCh38"){
      ...
      mito.genes <- grep("^mt-", rownames(GetAssayData(object = SeuratObj)), value = TRUE)
      ercc.genes <- grep("^ercc", rownames(GetAssayData(object = SeuratObj)), value = TRUE)}
    percent.mito <- (Matrix::colSums(GetAssayData(object = SeuratObj)[mito.genes, ])/Matrix::colSums(GetAssayData(object = SeuratObj)))*100
    percent.ercc <- (Matrix::colSums(GetAssayData(object = SeuratObj)[ercc.genes, ])/Matrix::colSums(GetAssayData(object = SeuratObj)))*100
