#' Convert scRNA-seq count matrix to TPM matrix
#'
#' Convert scRNA-seq count matrix to TPM matrix for downstream analysis.
#'
#' @docType methods
#' @name RNACountToTPM
#' @rdname RNACountToTPM
#'
#' @param countMat Input count matrix, with genes as rows and cells as columns.
#' @param idType Type of gene name, available options are "Ensembl" or "Symbol". Default is "Ensembl".
...
#'
#' @return A matrix of TPM, with genes as rows and cells as columns.
#'
#'
#' @examples
#' data(pbmc.RNA)
#' pbmc.RNA.TPM <- RNACountToTPM(pbmc.RNA, idType = "Ensembl", organism = "GRCh38")
#' head(pbmc.RNA.TPM[,1:10])
#'
#' @export
