...
def init_workflow(directory, mode):
    ...
    def get_resource(f,mode):
        if mode == "scRNA":
            return os.path.join(os.path.dirname(__file__), "scRNA", "Snakemake", f)
        if mode == "scATAC":
            return os.path.join(os.path.dirname(__file__), "scATAC", "Snakemake", f)
...
def main():
    # create arg parser
    parser = ap.ArgumentParser(description = "STRAP is a comprehensive quality control, analysis and visualization pipeline for scATAC-seq and scRNA-seq. ")
    parser.add_argument("-v", "--version", action = "store_true", help = "Print version info.")

    subparsers = parser.add_subparsers(dest = "subcommand")
    workflow = subparsers.add_parser("init", help = "Initialize the STRAP workflow in a given directory. "
