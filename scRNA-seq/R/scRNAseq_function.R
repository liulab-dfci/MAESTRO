...
{
  if(organism=="GRCh38")
    ensembl = read.delim(paste0(srcdir, "annotations/hg38_mart_export.txt"), check.names = FALSE)
  else
    ensembl = read.delim(paste0(srcdir, "annotations/mm10_mart_export.txt"), check.names = FALSE)
  ...
  dims.use = 1:15, marker.use = "markers.CIBERSORT", res = 0.6)
{
  ...
  png(file.path(paste0("tSNE_origIdent_", SeuratObj@project.name, "_cluster.png")), res=300, width=6, height=4, units = "in")
