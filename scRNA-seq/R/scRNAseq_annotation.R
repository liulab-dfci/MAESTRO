...
human_immune_TCIA <- c(human_immune_TCIA, human_stormal_simple)
...
human_immune_CIBERSORT <- c(human_immune_CIBERSORT, human_stormal_simple)
...
human_immune_BioLegend <- c(human_immune_BioLegend, human_stormal_simple)
...
TabulaMuris_droplet = sapply(1:length(TabulaMuris_droplet_celltype),function(i){return(as.character(TabulaMuris_droplet[TabulaMuris_droplet$cluster == TabulaMuris_droplet_celltype[i],2]))})
names(TabulaMuris_droplet) = TabulaMuris_droplet_celltype

TabulaMuris_droplet = read.table(paste0(srcdir,"annotations/TabulaMuris_facs_markers_all.txt"),header = TRUE, sep = '\t',check.names = FALSE)
TabulaMuris_facs_celltype = unique(TabulaMuris_facs$cluster)
TabulaMuris_facs = sapply(1:length(TabulaMuris_facs_celltype),function(i){return(as.character(TabulaMuris_facs[TabulaMuris_facs$cluster == TabulaMuris_facs_celltype[i],2]))})
