...
if os.path.isdir(lisa_path):
        if os.path.isdir(os.path.join(os.path.dirname(lisa.__file__), 'data', species)):
            print('lisa data already exists.')
        else:
            start_time = time.time()
            print('Start to unpack lisa data:',time.strftime("%a %b %d %H:%M:%S %Y", time.localtime()))
            cmd = "lisa unpack %s" %(lisadir)
            os.system(cmd)
            end_time = time.time()
