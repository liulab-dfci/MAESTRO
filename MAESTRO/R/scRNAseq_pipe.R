...
exp.dat = Read10X_h5(filename = argue[1], unique.features = TRUE)

plan("multiprocess", workers = as.integer(argue[6]))
options(future.globals.maxSize = 10*1024^3)

RNA.res <- RNARunSeurat(inputMat = exp.dat, project = argue[3], min.c = 10, min.g = 100)
RNA.res$RNA <- RNAAnnotateCelltype(RNA = RNA.res$RNA, genes = RNA.res$genes, signatures = human.immune.CIBERSORT, min.score = 0.05)
saveRDS(RNA.res$RNA, paste0(argue[3], "_scRNA_Object.rds"))
RNA.tfs <- RNAAnnotateTranscriptionFactor(RNA = RNA.res$RNA, genes = RNA.res$genes, project = argue[3], rabit.path = argue[4], organism = argue[2], top.tf = 10)
