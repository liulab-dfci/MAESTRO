...
runGIGGLE <- function(clusterPeaks, species, output){
  ...
  for (icluster in unique(clusterPeaks$cluster)) {
      ...
      write.table(iclusterPeaks, outputBed, sep="\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      targetDf <- targetDf[1:10, ]  
      write.table(targetDf, paste0(gsub("cluster_specific_peaks.bed", "", outputBed), "giggle_res_tfs.txt"), sep="\t", quote=FALSE, row.names=FALSE) 
      ...
      for (dcid in targetDf$sample_id) {
          tf <- as.character(targetDf[targetDf$sample_id==dcid, "factor"])
          tfTarget <- geneScore[["genes"]][geneScore[["indices"]][[dcid]]]
          write.table(data.frame(tfTarget), paste0(gsub("cluster_specific_peaks.bed", "", outputBed), tf, "_", dcid, "_target_genes_top500.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
          targetList[[icluster]][[tf]] <- tfTarget
      }
  }
  return(targetList)
}
