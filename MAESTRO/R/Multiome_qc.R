...
singlestat_file = "Result/ATAC/QC/singlecell.txt"
countgene_file = "Result/RNA/QC/PBMC_granulocyte_sorted_3k_count_gene_stat.txt"
count_file = "Result/Multiome/PBMC_granulocyte_sorted_3k_multiome_feature_count.h5"

name = "Result/Multiome/PBMC_granulocyte_sorted_3k"
MultiomeFilteringPlot(countgene_file, singlestat_file, count_file, name)
MultiomeFilteringPlot <- function(rna_statpath, atac_statpath, multiome_count, name){
  ...
  non_cells = all[!(cells %in% all)]

  png(paste0(name,"_multiome_cell_filtering.png"),width=4.8,height=4.8, res = 300, units = "in")
  par(mai = c(0.85, 0.85, 0.25, 0.25))
  plot(log10(UMI_gene[non_cells,1]+1),log10(mapping_matrix[non_cells,1]+1),
       xlim=c(0,log10(max(UMI_gene[,1])+1)+0.25), ylim=c(0,max(mapping_matrix[,1])+0.25), pch='.',col=RCB_red,ylab='ATAC transposition events in peaks per barcode',xlab='RNA UMIs per barcode (log10)')
